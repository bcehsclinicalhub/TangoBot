---
title: Calls in Evacuation Order Zones
policy_id: 
version: 1.0
status: Active
effective_date: 2024-12-01
last_reviewed: 2024-12-01
next_review: 2023-03-01
owner: Clinical Hub
category: Operational

tags:
  - Wildfire
  - Safety
  - DST

keywords:
  - Wildfire
  - Safety
  - Interface
  - Fire
  - Evac
  - Evacuation Zones
---

# Calls in Evacuation Order Zones <br> <span style="font-size: 0.8em; font-style: italic;">Process for assessing calls</span>


## Purpose

Workflow and process for assessing calls inside Evacuation Order Zones.


## Procedure

1. Calltaker assesses 911 call using MPDS. Calltaker notifies CEMD in area of responsibility and Paramedic Specialist.

2. Calltaker warm transfers call to Paramedic Specialist:

!!!NOTE "Exclusions for PS Transfer"

- CRM Purple

- Childbirth

- Maintain Airway PAI being used

**If any of these occur EMCT should remain on the line directly with caller**

!!!NOTE "PS Speed Dials"

- KDOC: S/D 666

- LDOC: S/D 333

- VDOC: Use DID

## Paramedic Specialist Assessment

Paramedic Specialist have a range of tools to help determine event acuity. Including discussing the call with the review requester, using clinical
judgement and patient call-back utilizing the Manchester triage system.

- PS assessment = patient **does not** require face to face assessment.

❑ Sig POM and Regional Charge Dispatcher/DS and Request to Cancel the event

- PS assessment =  patient **does** require face to face assessment.

❑ Conference call with PS, POM, CEMD, area Manager on Call

**Contact numbers:**

Coastal/Fraser MoC Pager: 855-340-2030

Island MoC Pager: 855-670-7883

Interior MoC Pager: 855-370-0570

North MoC Pager: 855-549-0551

May be given alternate contact by DEOC depending on event - contact area manager on call, unless informed otherwise.

## CEMD advises caller of plan

Either (i) no response or (ii) other agency responding. 

!!!NOTE "Recommended that call should be made by regional CEMD for event. If allied agency responding to patient, may be more appropriate
for allied agency to contact caller directly at discretion of CEMD and area manager on call."

**BCEHS crew meets patient/allied agency at designated meet point outside evacuation order zone.**

!!!NOTE "Recommend CIS activation for all staff involved in call"


## References

**Guidelines/Procedures/Forms**

[Working in Evacuation Order and Alert Areas](https://shop.healthcarebc.ca/phsa/BCEHS/Workplace%20Health/C-04-12-85310.pdf){:target="_blank .external-link}

[Paramedic Safety at the Scene](https://shop.healthcarebc.ca/phsa/BCEHS/Clinical%20Operations/C-04-11-85066.pdf){:target="_blank .external-link}

[Station Evacuation Procedure](https://handbook.bcehs.ca/operations/emergency-management/station-disaster-planning-and-response/station-evacuation-procedure/){:target="_blank .external-link}

## Review Schedule

| Adopted | Next Review Scheduled | Owner | Reviewer| 
| :--- | :--- | :--- | :--- | 
| Dec 2024 | Mar 2025 | Clinical Hub Mnager | BCEHS OHS Team |